...
export const listenToBusinessById = (businessId: string, callback: (business: Business | null) => void): (() => void) => {
    const docRef = businessesCollectionRef.doc(businessId);
    return docRef.onSnapshot(docSnap => {
        logUsage(businessId, { firestoreReads: 1 });
        if (docSnap.exists) {
...
export const addBusiness = async (businessData: Omit<Business, 'businessId' | 'products' | 'totalMessages'>): Promise<Business> => {
...
export const logChatMessage = async (businessId: string, sessionId: string, message: Omit<ChatMessage, 'id' | 'timestamp'>) => {
    const businessRef = businessesCollectionRef.doc(businessId);
    const sessionRef = businessRef.collection('chatSessions').doc(sessionId);
