                                    ...
                                    liveSessions.map(session => (
                                        <li key={session.id} className="flex items-center justify-between w-full text-left p-3 rounded-lg hover:bg-[var(--bg-secondary)]/80 transition-colors" style={{backgroundColor: 'var(--bg-secondary)'}}>
                                            <div>
                                                ...
                                                <p className="text-xs text-[var(--text-secondary)] truncate">{session.messages[0]?.text || 'Session started...'}</p>
                                            </div>
                                            <button onClick={() => setSessionToJoin(session)} className={`font-bold py-1 px-3 rounded-md text-xs transition-colors ${session.agentJoined ? 'bg-green-600 text-white' : 'btn-primary'}`}>
